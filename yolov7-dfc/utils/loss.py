...
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy ,shape_iou
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False,    CIoU=True) #scale=1) # iou(prediction, target)
                # lbox += (1.0 - iou).mean()  # iou loss

                # iou = shape_iou(pbox.T, tbox[i], x1y1x2y2=False,  scale=1.1) # iou(prediction, target)
                # lbox += (1.0 - iou).mean()  # iou loss
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #                 CIoU=True )  # , ) scale=1 # iou(prediction, target)
                # lbox += (1.0 - iou).mean()  # iou loss

                # iou = shape_iou(pbox.T, selected_tbox, x1y1x2y2=False,
                #                         scale=1.1) # iou(prediction, target)
                # lbox += (1.0 - iou).mean()  # iou loss

                # inner shape iou
                obj_sz = (tobj.size()[2], tobj.size()[3])
                iou = bbox_iou(pbox, selected_tbox, feat_sz=obj_sz, shape_iou=True, scale=1.2,eps=1e-7)
                if isinstance(iou, tuple):
                    if len(iou) == 2:
                        lbox += (iou[1].detach().squeeze() * (1 - iou[0].squeeze())).mean()
                        iou = iou[0].squeeze()
