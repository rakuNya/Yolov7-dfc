...
class Sim_DFC(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Sim_DFC, self).__init__()
        self.gate_fn = nn.Sigmoid()

        self.conv1 = Conv(inc, hidc, k=1, act=act) #180 x 180 x 32
        self.conv2 = Conv(inc, hidc, k=1, act=act) #180 x 180 x 32
        self.conv3 = Conv(inc, hidc, k=3, act=act) #180 x 180 x 32
        # self.Sim =  SimAM()  #180 x 180 x 32
        self.Sim = SimAM()
        self.conv4 = Conv(hidc * 2, hidc, k=1, act=act) #180 x 180 x 32
        self.conv5 = GhostConv(hidc * 2, hidc, k=3, act=act) #180 x 180 x 32
        self.conv6 = DepSepConv(hidc, hidc, k=3, act=act)     #180 x 180 x 32
        self.EMA = EMA(hidc, hidc)
        self.conv7 = GhostConv(hidc * 4, ouc, k=1, act=act)

    def forward(self, x):
        x1, x2, x3 = self.conv1(x), self.conv2(x), self.conv3(x)
        x_Sim = self.Sim(x1)
        x_concat1 = torch.concat([x2, x3], dim=1)
        x4 = self.conv4(x_concat1)
        x_mul = x4 * self.gate_fn(x_Sim)
        x5 = self.conv5(x_concat1)
        x6 = self.conv6(x5)
        x_add = x6 + x_mul
        x_ema = self.EMA(x_add)
        x_concat2 = torch.concat([x_Sim, x_mul, x5, x_ema], dim=1)
        x_final = self.conv7(x_concat2)
        return x_final
##### end of Sim_DFC #####
...
class GhostModuleV2(nn.Module):
    ...
    def forward(self, x):
        if self.mode in ['original']:
            ...
            res = self.shot_conv(F.avg_pool2d(x, kernel_size=2, stride=2))
            x1 = self.primary_conv(x)
            x2 = self.cheap_operation(x1)
            out = torch.cat([x1, x2], dim=1)
...
class EMA(nn.Module):
    def __init__(self, channels, c2=None, k=3, s=None, factor=8):
        ...
        self.conv = Conv(channels, c2, k, s)
    def forward(self, x):
        b, c, h, w = x.size()
...
class DepSepConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, act=True):
        ...
        # 深度卷积层
        self.depthwise = nn.Sequential(nn.Conv2d(c1, c1, k,
                                                 s, autopad(k, p), groups=c1),
                                       nn.BatchNorm2d(c1),
                                       # activation_layer
                                       nn.SiLU()
                                       )
        # 逐点卷积层
        self.pointwise = nn.Sequential(nn.Conv2d(c1, c2, 1),
                                       nn.BatchNorm2d(c2),
                                       # activation_layer
                                       nn.SiLU()
                                       )
